...
- `type`: `stock` 表示 A 股，`gold` 表示现货黄金。
- `code`: 股票使用腾讯行情接口代码，深市用 `sz` 开头，沪市用 `sh` 开头；现货黄金使用 `hf_XAU`。
- `name`: 自定义显示名称。
- `targets`: 一个标的可以配置多个提醒价格。
- `price`: 目标价格。股票单位为元，黄金单位为元/克。
- `direction`: `below` 表示当前价低于或等于目标价时提醒，`above` 表示当前价高于或等于目标价时提醒。
- `label`: 邮件标题中的提醒标签。

黄金价格来源为腾讯 `hf_XAU` 现货黄金美元/盎司报价，并使用公开 USD/CNY 汇率折算为人民币/克。该价格与上海黄金交易所人民币金价可能存在少量价差，适合作为提醒参考。
